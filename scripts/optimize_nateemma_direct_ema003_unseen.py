"""Optimize EMA003 on a training window, then retest on unseen data."""
...
MODULE_NAME = "Strategy_Import_Nateemma_Direct_EMA003"
...
PARAM_GRID = {
    "ema003_short_period": [5, 7, 9],
    "ema003_long_period": [21, 25, 34],
    "trend_ma_period": [34, 50, 72],
    "trend_deadband_pct": [0.05, 0.10, 0.15],
}

DEFAULT_PARAMS = {"ema003_short_period": 7, "ema003_long_period": 25, "trend_ma_period": 50, "trend_deadband_pct": 0.15}
...
def _run_trial(params: dict, start_date: pd.Timestamp, end_date: pd.Timestamp, label: str) -> dict:
    if params["ema003_long_period"] <= params["ema003_short_period"]:
        return {"strategy_module": MODULE_NAME, "label": label, "params": params, "start_date": str(start_date), "end_date": str(end_date), "total_return_pct": -9999.0, "win_rate_pct": 0.0, "sharpe_ratio": -9999.0, "max_drawdown_pct": 9999.0, "total_trades": 0}
...
def main():
    ...
    rows = [row for row in rows if row["total_return_pct"] > -9000.0]
    ...
    json_path = out_dir / "github_nateemma_direct_ema003_unseen_optimization.json"
    md_path = out_dir / "github_nateemma_direct_ema003_unseen_optimization.md"
    json_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    md_path.write_text(
        "\n".join(
            [
                "# EMA003 Unseen-Data Optimization Results",
                "",
