"""Optimize EMA003 on the standard 10-symbol / 31-day screen."""
...
SHORT_PERIODS = [5, 7, 9]
LONG_PERIODS = [21, 25, 34]
PARAM_GRID = [
    {
        "ema003_short_period": short_period,
        "ema003_long_period": long_period,
        "trend_ma_period": trend_ma,
        "trend_deadband_pct": deadband,
    }
    for short_period in SHORT_PERIODS
    for long_period in LONG_PERIODS
    if long_period > short_period
    for trend_ma in [34, 50, 72]
...
def main():
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = [executor.submit(_run_trial, params, start_date, end_date) for params in PARAM_GRID]
    ...
    json_path = output_dir / "github_nateemma_direct_ema003_optimization.json"
    md_path = output_dir / "github_nateemma_direct_ema003_optimization.md"
