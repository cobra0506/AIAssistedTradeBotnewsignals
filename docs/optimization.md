...
4. Persist best parameters (with `last_optimized` date) via `ParameterManager` when `save_params=True`.
...
Walk-forward trains on one time window, then tests on the next window and repeats.
...
Results are saved to `simple_strategy/optimization_results/walk_forward_results.json`.
