...
class RiskManager:
    ...
    def validate_trade_signal(self, signal: Dict[str, Any], account_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            valid_types = ['OPEN_LONG', 'OPEN_SHORT', 'CLOSE_LONG', 'CLOSE_SHORT']
            ...
            def _get_position_key():
                if timeframe is not None and (symbol, timeframe) in positions:
            ...
            # For OPEN signals, check if we can open a new position
            if signal_type in ('OPEN_LONG', 'OPEN_SHORT'):
                ...
                if _get_position_key() is not None:
                    result['valid'] = False
            ...
            # For CLOSE signals, check if we have a position to close
            elif signal_type in ('CLOSE_LONG', 'CLOSE_SHORT'):
                position_key = _get_position_key()
                ...
                if signal_type == 'CLOSE_LONG' and is_short:
                    result['valid'] = False
                    result['reason'] = f"Cannot CLOSE_LONG for short position {symbol}"
                    return result
                if signal_type == 'CLOSE_SHORT' and not is_short:
                    result['valid'] = False
                    result['reason'] = f"Cannot CLOSE_SHORT for long position {symbol}"
                    return result
